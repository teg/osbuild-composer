...
func NewServer(workers *worker.Server, rpmMetadata rpmmd.RPMMD, distros *distro.Registry, credentials *koji.GSSAPICredentials) *Server {
	server := &Server{
		workers:     workers,
...
func (server *Server) PostCompose(w http.ResponseWriter, r *http.Request) {
	...
	k, err := koji.NewFromGSSAPI(request.Koji.Server, server.credentials, transport)
	if err != nil {
