...
func (q *fsJobQueue) Dequeue(ctx context.Context, jobTypes []string, args interface{}) (uuid.UUID, []uuid.UUID, error) {
	q.mu.Lock()
	defer q.mu.Unlock()
	...
	if err != nil {
		return uuid.Nil, nil, fmt.Errorf("error unmarshaling arguments for job '%s': %v", j.Id, err)
	}
