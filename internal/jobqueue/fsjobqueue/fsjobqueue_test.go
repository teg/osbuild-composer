...
func finishNextTestJob(t *testing.T, q jobqueue.JobQueue, jobType string, result interface{}, deps []uuid.UUID) uuid.UUID {
	id, d, err := q.Dequeue(context.Background(), []string{jobType}, &json.RawMessage{})
	require.NoError(t, err)
	require.NotEmpty(t, id)
...
func TestArgs(t *testing.T) {
	...
	var args argument
	id, deps, err := q.Dequeue(context.Background(), []string{"octopus"}, &args)
	require.NoError(t, err)
	require.Equal(t, two, id)
	require.Equal(t, twoargs, args)
	require.Empty(t, deps)

	id, deps, err = q.Dequeue(context.Background(), []string{"fish"}, &args)
	require.NoError(t, err)
...
func TestJobTypes(t *testing.T) {
	...
	cancel()
	id, deps, err := q.Dequeue(ctx, []string{"zebra"}, nil)
...
func TestMultipleWorkers(t *testing.T) {
	...
	go func() {
		...
		defer cancel()
		id, deps, err := q.Dequeue(ctx, []string{"octopus"}, &json.RawMessage{})
		require.NoError(t, err)
		require.NotEmpty(t, id)
		require.Empty(t, deps)
	}()
	...
	r, deps, err := q.Dequeue(context.Background(), []string{"clownfish"}, &json.RawMessage{})
	require.NoError(t, err)
